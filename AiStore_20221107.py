class C_AiStore:
    ...
    def __init__(self, store_name, store_id, store_address, product_stock, product_price, return_money):
    ...
    def set_product(self, product_price):
        info = input('상품개수: ')
        self.product_price = product_price*info


    # 구매개수, 입력금액(지불할돈)
    def buy_item(self, order_unit):
        ...
        if self.input_money < self.product_price:
            print('금액이 부족합니다')

        self.return_money = self.product_price - self.input_money
            print('잔돈은', self.return_money , '입니다')
...
if __name__ == '__main__':
    ...
    aistore = C_AiStore(s_name, s_id, s_locate, s_stock, s_price, s_money)
    ...
    for button in range(0, 10):
        ...
        if button == 1:
            ...
            info = input('구매할 상품 개수 입력: ')
            # aistore.buy_item(info)

            print('상품 가격은 ', aistore.product_price(info), '입니다')

            # info = int(input('필요한 금액 출력: '))
            print('잔돈은', aistore.buy_item(info) , '입니다')
            # aistore.buy_item(info)
